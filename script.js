...
function showAll() {
    ...
    updateFilterButtons();
    renderTodos();
    updateTaskListHeader('All Tasks', todos.length);
}
...
function showPending() {
    ...
    updateTaskListHeader('Pending Tasks', pendingCount);
}
...
function showCompleted() {
    ...
    updateTaskListHeader('Completed Tasks', completedCount);
}
...
function showByPriority(priority) {
    ...
    updateFilterButtons();
    renderTodos();
    const priorityCount = todos.filter(todo => todo.priority === priority).length;
    updateTaskListHeader(`${priority.charAt(0).toUpperCase() + priority.slice(1)} Priority Tasks`, priorityCount);
}
...
function filterByCategory(category) {
    ...
    renderTodos();
    if (category === 'all') {
        updateTaskListHeader('All Categories', todos.length);
    } else {
        const categoryCount = todos.filter(todo => todo.category === category).length;
        updateTaskListHeader(`${category.charAt(0).toUpperCase() + category.slice(1)} Tasks`, categoryCount);
    }
}
...
function showOverdue() {
    ...
    updateTaskListHeader('Overdue Tasks', overdueCount);
}
...
// Show all tasks directly in the main list
function showAllTasksInList() {
    ...
    showNotification('📋 Showing all tasks in main list', 'info');
}

// Update task list header with current filter info
function updateTaskListHeader(title, count) {
    const titleElement = document.getElementById('taskListTitle');
    const countElement = document.getElementById('currentTaskCount');
    
    if (titleElement) {
        titleElement.innerHTML = `<i class="fas fa-tasks"></i> ${title}`;
    }
...
// Update existing filter functions to use the new header
function updateTaskCount() {
    const visibleTasks = document.querySelectorAll('#todoList .todo-item:not([style*="display: none"])').length;
    updateTaskListHeader(getCurrentFilterTitle(), visibleTasks);
}

function getCurrentFilterTitle() {
    if (currentFilter === 'completed') return 'Completed Tasks';
    if (currentFilter === 'pending') return 'Pending Tasks';
    if (currentFilter === 'high') return 'High Priority Tasks';
    if (currentFilter === 'overdue') return 'Overdue Tasks';
    if (currentCategoryFilter && currentCategoryFilter !== 'all') {
        return `${currentCategoryFilter.charAt(0).toUpperCase() + currentCategoryFilter.slice(1)} Tasks`;
    }
    return 'All Tasks';
}
...
function renderOverviewTasks() {
    ...
    if (filteredTodos.length === 0) {
        tasksList.innerHTML = `
            <div class="overview-empty-state" style="text-align: center; padding: 30px; background: rgba(255, 255, 255, 0.05); border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.1);">
...
function showCustomTemplateModal() {
    document.getElementById('customTemplateModal').classList.remove('hidden');
    document.getElementById('templateName').value = '';
    document.getElementById('templateDescription').value = '';
    document.getElementById('templateContent').value = '';
    document.getElementById('selectedTemplateIcon').value = '📝';
...
function closeCustomTemplateModal() {
    document.getElementById('customTemplateModal').classList.add('hidden');
}

// Select template icon
function selectTemplateIcon(icon) {
    document.getElementById('selectedTemplateIcon').value = icon;
...
function previewCustomTemplate() {
    ...
    if (!name || !content) {
        showNotification('❌ Please fill in template name and content', 'error');
        return;
    }
    
    // Process placeholders
    const processedContent = processTemplatePlaceholders(content);
    ...
    openNoteEditor(previewNote);
    showNotification('👀 Template preview opened!', 'info');
}

// Process template placeholders
function processTemplatePlaceholders(content) {
    const now = new Date();
    ...
    return content
        ...
        .replace(/\[Title\]/g, '[Enter Title Here]')
        .replace(/\[Week\]/g, `Week ${Math.ceil((now.getDate()) / 7)}`)
...
function saveCustomTemplate() {
    ...
    const content = document.getElementById('templateContent').value.trim();
    const icon = document.getElementById('selectedTemplateIcon').value;
    ...
    if (customTemplates.some(t => t.name.toLowerCase() === name.toLowerCase())) {
        showNotification('❌ Template name already exists', 'error');
        return;
    }
...
function renderCustomTemplates() {
    ...
    customTemplates.forEach(template => {
        ...
        templateCard.onclick = () => createNoteFromCustomTemplate(template.id);
        ...
        templateCard.innerHTML = `
            ...
            <button class="delete-custom-template" onclick="event.stopPropagation(); deleteCustomTemplate(${template.id})" title="Delete Template">
...
// Create note from custom template
function createNoteFromCustomTemplate(templateId) {
    ...
    showNotification(`📝 Note created from "${template.name}" template!`, 'success');
    playSound('add');
}
...
function deleteCustomTemplate(templateId) {
    ...
    renderCustomTemplates();
    showNotification('🗑️ Template deleted successfully', 'success');
    playSound('delete');
}
...
window.showCustomTemplateModal = showCustomTemplateModal;
...
window.createNoteFromCustomTemplate = createNoteFromCustomTemplate;
window.deleteCustomTemplate = deleteCustomTemplate;
window.showAllTasksInList = showAllTasksInList;